...
- Make your joins safer with the `check` argument and the
  [`check_specs()`](reference/check_specs.md)function
...
The `check` argument receives an object created by the
[`check_specs()`](reference/check_specs.md) function, which provides
ways to handle specific input properties, its arguments can be :
...
[`coalesce_xy()`](reference/coalesce_xy.md) and
[`coalesce_yx()`](reference/coalesce_xy.md) to ease this task (both
wrapped around
...
[`pack_along_keys()`](reference/preprocess_inputs.md) packs given
columns, or all non key columns by default, into a data frame column
named by the `name` argument, it’s useful to namespace the data and
avoid conflicts
...
- [`nest_by_keys()`](reference/preprocess_inputs.md) nests given
  columns, or all by default, if `name` is given a single list column of
  data frames is created
- [`complete_keys()`](reference/preprocess_inputs.md) expands the key
  columns, so all combinations are present, filling the rest of the new
  rows with `NA`s. Absent factor levels are expanded as well.
